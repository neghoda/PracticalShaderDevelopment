...
void Sample::Render(float inAspectRatio) {
	...
	Uniform<vec3>::Set(mShader->GetUniform("ambientColor"), vec3(0, 0, 0));
	Uniform<vec3>::Set(mShader->GetUniform("cameraPosition"), cameraPosition);

	Uniform<vec3>::Set(mShader->GetUniform("directionalLights[0].direction"), vec3(0, 5, 0));
	Uniform<vec4>::Set(mShader->GetUniform("directionalLights[0].color"), vec4(1, 1, 1, 1));

	Uniform<vec3>::Set(mShader->GetUniform("pointLights[0].position"), vec3(0, sin(mTime), 0.25));
	Uniform<vec3>::Set(mShader->GetUniform("pointLights[0].color"), vec3(1, 0, 0));
	Uniform<float>::Set(mShader->GetUniform("pointLights[0].radius"), 1);

	Uniform<vec3>::Set(mShader->GetUniform("pointLights[1].position"), vec3(sin(mTime), 0, 0.25));
	Uniform<vec3>::Set(mShader->GetUniform("pointLights[1].color"), vec3(0, 1, 0));
	Uniform<float>::Set(mShader->GetUniform("pointLights[1].radius"), 1);
	...
	Uniform<vec4>::Set(mWaterShader->GetUniform("ambientColor"), vec4(0, 0, 0, 1));
	...
	Uniform<vec3>::Set(mWaterShader->GetUniform("directionalLights[0].direction"), vec3(0, 5, 0));
	Uniform<vec4>::Set(mWaterShader->GetUniform("directionalLights[0].color"), vec4(1, 1, 1, 1));

	Uniform<vec3>::Set(mWaterShader->GetUniform("pointLights[0].position"), vec3(0, sin(mTime), 0.25));
	Uniform<vec3>::Set(mWaterShader->GetUniform("pointLights[0].color"), vec3(1, 0, 0));
	Uniform<float>::Set(mWaterShader->GetUniform("pointLights[0].radius"), 1);

	Uniform<vec3>::Set(mWaterShader->GetUniform("pointLights[1].position"), vec3(sin(mTime), 0, 0.25));
	Uniform<vec3>::Set(mWaterShader->GetUniform("pointLights[1].color"), vec3(0, 1, 0));
	Uniform<float>::Set(mWaterShader->GetUniform("pointLights[1].radius"), 1);
