...
void Sample::Render(float inAspectRatio) {
	...
	Uniform<vec3>::Set(mShader->GetUniform("lightPos"), vec3(0, 0, -1));
	Uniform<vec3>::Set(mShader->GetUniform("lightConeDir"), vec3(0, 0, 1));
	Uniform<float>::Set(mShader->GetUniform("lightCutoff"), cos(30 * DEG2RAD));
